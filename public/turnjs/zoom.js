...
(function ($) {
    ...
    zoomMethods = {
        ...
        if (typeof event != 'undefined') {
          if ('x' in event && 'y' in event) {
            ...
            pos = $.isTouch
              ...
              : point2D(event.pageX - flipPos.x - thisOffset.left, event.pageY - flipPos.y - thisOffset.top);
          }
        } else {
      ...
      zoomOut: function (duration) {
        ...
        if (autoCenter && flip.turn('display') == 'double') {
          if (!flip.turn('view')[0])
            flipDesPos.x =
              flip.turn('direction') == 'ltr' ? flipDesPos.x - tmp.width() / 4 : flipDesPos.x + tmp.width() / 4;
      ...
      _eTurned: function (event, page) {
        if (this.zoom('value') != 1) {
          ...
          if (page > data.page) this.zoom('scroll', point2D(0, data.scrollPos.y), false, true);
          else if (page < data.page) this.zoom('scroll', point2D(flip.width(), data.scrollPos.y), false, true);
        }
      },
      ...
      _eMouseDown: function (event) {
        ...
        data.draggingCur = $.isTouch
          ? point2D(event.originalEvent.touches[0].pageX, event.originalEvent.touches[0].pageY)
  ...
  $.extend($.fn, {
    zoom: function () {
      ...
      if (!args[0] || typeof args[0] == 'object') return zoomMethods.init.apply($(this[0]), args);
      else if (zoomMethods[args[0]]) return zoomMethods[args[0]].apply($(this[0]), Array.prototype.slice.call(args, 1));
